...
This chapter deals with methodology, while numerical values are introduced in Chapter \ref{chapter_experimental_setup}, Experimental Setup. In the first section of this chapter, we present an overview of the solution approach suggested. In Section \ref{Player_Performance}, three different methods for predicting player performance are described. Finally, in the Sections \ref{Ch.5_Game_chips} and \ref{Ch.5_Variance_tradeoff} respectively, methods for implementing gamechips and handling risk are proposed.


\section{Solution Approach}

As stochastic programming models seek to leverage probability distribution of uncertain parameters, they are regarded as more computational demanding than deterministic programming models \citep{Shapiro}. A common method for solving a stochastic model is by use of scenario generation. Solving the model over an entire season, this approach is not computational feasible. The number of scenarios explodes even for a modest number of scenarios. With 2 scenarios in a gameweek, the number of scenarios already reaches over 1 million for only 20 gameweeks, $2^{20} = 1 048 576$. Hence, the FTCP is solved by a deterministic approach. It is solved in a rolling horizon framework where decisions are implemented for one gameweek at a time. When decisions are optimized for a given gameweek, a certain number of gameweeks ahead is considered. 

The mathematical model is adjusted for strategies for when to use gamechips and the element of risk handling is added. In addition, every time the decisions are optimized for a given gameweek

for gamechip strategies and risk handling. In addition, every time the decisions are optimized for a given gameweek 


\newpar

\begin{comment}
I suggest you reorganize the material in the first part of this section. You have the right content but the order is confusing. 
First: start by saying that you do not solve the stochastic program you presented. Say that you solve the optimization model in a rolling-horizon framework in order to make decisions for one gw at a time. 
Second: say that every time you optimize for a given gw you consider a certain number of gws ahead of the focal gw. 
Third: say that every time you optimize for a given gw you provide as input what you show in Figure 5.1.
\end{comment}
...
As described in Section \ref{Other_Relevant_Research}, there is a large resemblance between the Football Team Composition Problem and the well known Markowitz portfolio selection problem in financial theory. This is taken advantage of as constraints related to handling of risk are incorporated by use of empirical variance and correlation. 

\subsection{Rolling horizon}

The Fantasy Team Composition Problem is solved by a rolling horizon heuristic. The idea behind a rolling horizon heuristic is to split the problem into several sub-problems along a time axis, and then solve them in a chronological order taking into account the solution of the previous sub-problem. The heuristic suits problems with a long planning horizon where there are limited dependency between decisions made early and late in the planning horizon. Industrial planning and scheduling problems are common areas where the heuristic is used. The most important considerations are the number of sub-problems versus the length of each sub-problem, and which decisions to fix in each sub-problem. 
...
In the rolling horizon heuristic, each sub-problem is solved over a sub-horizon. This corresponds well to the FTCP, as FPL managers often consider several gameweeks when making decisions. Each sub-problem is split into three parts:
\begin{itemize}
...
\textit{Step 1}. For the set of gameweeks in the sub-horizon:  generate predictions of players' expected points, calculate empirical variance and correlation. 
\newline
...
\While{(t = t+1) $\leq$ $\mathcal{T}$}
    \State{Generate input data: expected points, variance and correlation for $TP_{t}$}
    \If{any gw in $TP_{t}$ is \textit{double} or \textit{blank}}
        \State{Implement gamechip strategy}
    \ElsIf{$TP_{t}$ reach last part of first quarter of season}
...
A second way of predicting Fantasy Premier League points is by use of regression. Before the regression-based prediction model is introduced, a brief discussion of evaluation of forecasting accuracy is presented.


\subsubsection{Training and test sets}
In order to determine the accuracy of a forecast, out-of-sample forecasts should be evaluated. To accomplish this, available data are separated into two portions: training and test data. A model is first fitted using the training data only. Then, the model's forecast accuracy is determined by comparing forecasts from the model with actual realizations contained in the test data. 
...
Forecast errors (Equation \ref{eq:f_err}) are on the same scale as the data \citep{Hyndman}. The two most commonly used scale-dependent measures are the Mean Absolute Error (MAE, Equation \ref{eq:MAE}) and Root Mean Squared Error (RMSE, Equation \ref{eq:RMSE}).
...
The Ljung-Box test is statistical test that is often conducted in order to test for autocorrelation in a time series for different lags \citep{Hyndman}. The reader is referred to \cite{Hyndman} for a detailed description of the test.
...
In order to decide the value of $\lambda$, and subsequently what variables to include in the model, a test and a training set is created. Furthermore, different models with different values of $\lambda$ are fitted on the training set. Moreover, predictions based on linear regression for each model associated with a distinct value of $\lambda$ is generated. Finally, the model with lowest RMSE is selected. That is, the variables associated with a non-zero $\beta$ are selected.

\subsubsection{Model Selection}

We have decided to model the variables deemed significant by the lasso-regression by use of \textit{linear regression}. A linear regression model an advantage compared to for instance logistic regressions, as points can be negative, which frequently happens in FPL. Moreover, the model and its coefficients are easily interpreted. As the observant reader might have noticed, only one of the explanatory variables presented, namely previous points, is explicitly related to points in previous rounds. This is done in order to limit already complicated dependencies between the explanatory variables. For instance, total points in a season so far would be highly correlated with goals scored so far in a season. However, some dependencies are still likely to exist between the variables, for instance between minutes played and goals scored. As the explanatory variables are assumed independent in a linear regression, this is a limitation to the method.  Modeling such dependencies is an interesting further development, but beyond the scope of this thesis. 
...
In our case, the mathematical model optimizes over the starting line-up variable, $y$. Hence, we can rewrite equation \ref{eq:port_var} by utilizing the fact that each portfolio weight is either $\frac{1}{11}$ or $0$. If we also substitute the expression for covariance with correlation, the expression simplifies to
